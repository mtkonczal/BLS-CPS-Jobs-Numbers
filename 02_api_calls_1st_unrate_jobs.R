...
##### TITLES #####
title1 <- "Unemployment Back Down in June"
title2 <- "Job Gains Steady in June"
title3 <- "Still Notable Diffusion Index at 50%"
...
unrate %>%
  ...
  mutate(dateTag = if_else(date >= max(date) %m-% months(3) | date == max(date) %m-% months(12), round(unrate, 4), NA)) %>%
