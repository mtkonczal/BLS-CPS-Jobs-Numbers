...
young_unrate <- young_unrate %>%
  mutate(
    men_unrate_20_24 = as.numeric(LNS14000037),
    date = as.Date(paste0(year, "/", month, "/", 1)),
    women_unrate_20_24 = as.numeric(LNS14000038)
  )
...
young_unrate %>%
  ...
  pivot_longer(
    c(men_unrate_20_24, women_unrate_20_24),
    names_to = "type",
    values_to = "value"
  ) %>%
  mutate(
    value = value / 100,
    type = if_else(
      type == "men_unrate_20_24",
      "Men, 20-24, Unemployment",
      "Female, 20-24, Unemployment"
    ),
    dateTag = if_else(
      date == max(date) | date == as.Date("2019-12-01"),
      value,
      NA_real_
    ),
    pointTag = if_else(
  ...
  labs(
    ...
    caption = "BLS. Mike Konczal, Economic Security Project."
  ) +
  scale_x_date(date_labels = "%b\n%Y", breaks = MI_dates) +
  theme(legend.position = "top")

ggsave(
  "graphics/g2a_young_unemployment_gender.png",
  dpi = "retina",
  width = 12,
  height = 6.75,
  units = "in"
