...
# Latest year/month with any estimate
latest_year <- revisions_df %>%
...
revisions_long <- revisions_df %>%
  filter(year == latest_year, month_num >= start_m, month_num <= latest_m) %>%
  transmute(
    month = factor(
      month_num,
