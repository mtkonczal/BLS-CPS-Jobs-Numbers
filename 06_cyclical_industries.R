...
diffusion_jobs <- ces_data %>% filter(data_type_code == 1,
...
MI_datesCYC <- sort(ces_data %>% filter(seasonal == "S") %>% filter(date <= max(date) %m-% months(1)) %>% distinct(date) %>% pull(), decreasing = TRUE)
MI_datesCYC <- MI_datesCYC[seq(1, length(MI_datesCYC), 48)]

# Job growth by sensititvity: 1 month ----
ces_data %>% filter(seasonal == "S",
               data_type_code == 1,
  ...
  labs(subtitle = "Three month average job gain by cyclical sensitivity. 2020-2021 removed.",
       y = "",
       x = "",
       title = title_cyc,
       caption = "250 Diffusion sub-industries 6-month job gains correlated against unemployment changes on 1991 to 2019 data. BLS. Mike Konczal") +
...
ggsave("graphics/g8_cyclical_industries.png", dpi = "retina", width = 12, height = 6.75, units = "in")
