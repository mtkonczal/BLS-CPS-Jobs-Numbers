...
unrate %>%
  ...
  mutate(
    dateTag = if_else(
      date >= max(date) %m-% months(4) | date == "2024-12-01",
      round(unrate, 4),
      NA
    )
  ) %>%
...
ggsave(
  "graphics/federal_versus_rest.png",
  dpi = "retina",
  width = 12,
  height = 6.75,
  units = "in"
...
ggsave(
  "graphics/g3_diffusion_index.png",
  dpi = "retina",
  width = 12,
  height = 6.75,
  units = "in"
...
# Sahm Rule Graphic -----
df <- make_Sahm(unrate %>% rename(unemployment = unrate)) %>%
...
ggsave(
  "graphics/jobs_gained_fixed.png",
  dpi = "retina",
  width = 12,
  height = 6.75,
  units = "in"
